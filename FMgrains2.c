...
Carrier Frequency and Grain Duration are aleatoric while Modulator Frequency remains constant.
Darien Brito, 2014.
*/
...
int C74_EXPORT main(void) {
    ...
    c = class_new("FMgrains2~", (method) FMNew, (method) FMfree, sizeof(fmsynth), 0L, A_GIMME, 0);
...
void *FMNew (double minfreq, double maxfreq, double modFreq, double dex, double minwin, double maxwin){
    fmsynth *x;
    ...
    floatin((t_object *) x, 5);                 // Inlet for max grain val
    floatin((t_object *) x, 4);                 // Inlet for min grain val
    floatin((t_object *) x, 3);                 // Inlet for Modulator Depth
    floatin((t_object *) x, 2);                 // Inlet for Modulator
...
void FMPerform64 (fmsynth *x, t_object *dsp64, double **ins, long numins, double **outs, long numouts, long sampleframes, long flags, void *userparam) {
	t_double  *out = outs[0];
	double increment,increment2,incrementAmp,index,*table, *modtable, calc, index2, mod,modindex, sampleRate, *window, indexAmp, winFlag, env,min,max,freq, minFreq, maxFreq;
